...
public class ContributedGemRegistry
{
	...
	public static Collection<Gem> getContributedGems()
	{
		if (fContributed == null)
		{
			...
			IExtensionPoint extensionPoint = Platform.getExtensionRegistry().getExtensionPoint(
					AptanaRDTPlugin.PLUGIN_ID, AptanaRDTPlugin.EXTENSION_POINT_GEMS);
	...
	private static Gem createGem(IConfigurationElement element)
	{
		...
		String autoInstall = element.getAttribute("auto-install"); //$NON-NLS-1$
		...
		{
			gem = new Gem(name, getVersion(element), null);
		}
		else
		{
			gem = createLocalGem(name, path, element);
		}
		String compiles = element.getAttribute("compiles");
		gem.setCompiles(Boolean.parseBoolean(compiles));
		
		boolean forceUpdate = false;
		String forceUpdateRaw = element.getAttribute("force-update"); //$NON-NLS-1$
	...
	private static Gem createLocalGem(String name, String path, IConfigurationElement element)
	{
		...
		if (url == null)
		{
			AptanaRDTPlugin.log("Unable to generate a URL for the gem with path: " + path + " in the "
					+ element.getContributor().getName() + " plugin");
			return null;
		}
		Gem gem = null;
		...
		else
		{
			...
			gem = new LocalFileGem(url, name, getVersion(element), "", platform);
		}
		IConfigurationElement[] dependencies = element.getChildren("dependency"); //$NON-NLS-1$
		for (int j = 0; j < dependencies.length; j++)
		{
			String dependency = dependencies[j].getAttribute("name"); //$NON-NLS-1$
			((LocalFileGem) gem).addDependency(dependency);
		}
		return gem;
	}
	...
	public static Gem getGem(String name)
	{
		...
		for (Gem gem : gems)
		{
			...
			{ // TODO Find latest version of gem
				return gem;
			}
		}
		return null;
	...
	public static Collection<Gem> filterByPlatform(Collection<Gem> gems)
	{
		...
		for (Gem gem : gems)
		{
			if (map.containsKey(gem.getName()))
			{
				...
				// it
				if (RubyRuntime.currentVMIsJRuby() && gem.getPlatform().equals(Gem.JRUBY_PLATFORM))
				{
					map.put(gem.getName(), gem);
				}
				// if we're on windows, not using JRuby, and this is the windows
				// specific gem, use it
				if (!RubyRuntime.currentVMIsJRuby() && Platform.getOS().equals(Platform.OS_WIN32)
			...
			for (Gem gem : gems)
			{
				...
				if (gem instanceof LocalFileGem)
				{
					...
					for (String dependency : dependencies)
					{
						...
						if (!contains(sorted, dependency))
						{ // if it's not ahead of us in list to be installed, skip us for now
							add = false;
							if (!contains(gems, dependency))
							{ // it's not installed, ahead of us, or available - we're screwed, remove this gem from list to be installed
								AptanaRDTPlugin.log("Unable to install " + local.toString()
