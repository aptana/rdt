...
public class TypeHierarchy implements ITypeHierarchy, IElementChangedListener
{
	...
	/**
	 * The Ruby Project in which the hierarchy is being built - this provides the context for determining a classpath
	 * and namelookup rules. Possibly null.
	 */
	...
	protected Map<IType, IType[]> typeToSuperModules;
	...
	protected ArrayList<IType> modules = new ArrayList<IType>(10);
	...
	{
		this.focusType = type == null ? null : (IType) ((RubyElement) type).unresolved(); // unsure the focus type is
		// unresolved (see
	...
	/**
	 * Adds all of the elements in the collection to the list if the element is not already in the list.
	 */
	...
	/**
	 * Caches the handle of the superclass for the specified type. As a side effect cache this type as a subtype of the
	 * superclass.
	 */
	...
	/**
	 * Caches all of the supermodules that are specified for the type.
	 */
	...
	/**
	 * Checks with the progress monitor to see whether the creation of the type hierarchy should be canceled. Should be
	 * regularly called so that the user can cancel.
	 * 
	...
	/**
	 * Notifies listeners that this hierarchy has changed and needs refreshing. Note that listeners can be removed as we
	...
	public void fireChange()
	{
		...
		if (DEBUG)
		{
			...
			{
				System.out
						.println("    for hierarchy focused on " + ((RubyElement) this.focusType).toStringWithAncestors()); //$NON-NLS-1$
			}
		}
	...
	public IType[] getAllClasses()
	{
		...
		for (IType type : this.classToSuperclass.keySet())
		{
			classes.add(type);
		}
 ...
 */
	...
	private void getAllSuperModules0(IType type, ArrayList<IType> supers)
	{
		IType[] superinterfaces = this.typeToSuperModules.get(type);
	...
	private IType[] getExtendingModules0(IType extendedInterface)
	{
		...
		for (IType type : this.typeToSuperModules.keySet())
		{
			if (!this.isModule(type))
			{
				continue;
	...
	private IType[] getIncludingClasses0(IType interfce)
	{
		...
		for (IType type : this.typeToSuperModules.keySet())
		{
			...
			IType[] types = this.typeToSuperModules.get(type);
	...
	/**
	 * Returns whether one of the subtypes in this hierarchy has the given simple name or this type has the given simple
	 * name.
	 */
	...
	/**
	 * Returns whether the simple name of the given type or one of its supertypes is the simple name of one of the types
	 * in this hierarchy.
	 */
	...
	protected void initialize(int size)
	{
		...
		this.modules = new ArrayList<IType>(smallSize);
		...
		this.typeToSuperModules = new HashMap<IType, IType[]>(smallSize);
	...
	/**
	 * Returns true if any of the children of a project, package fragment root, or package fragment have changed in a
	 * way that effects this type hierarchy.
	 */
		...
		switch (delta.getKind())
		{
			...
			case IRubyElementDelta.CHANGED:
				...
				if ((flags & IRubyElementDelta.F_ADDED_TO_CLASSPATH) > 0)
				{
					...
					if (this.projectRegion != null)
					{
						...
						for (int i = 0; i < elements.length; i++)
						{
							...
							try
							{
								ILoadpathEntry[] classpath = javaProject.getResolvedLoadpath(
				...
				if ((flags & IRubyElementDelta.F_REMOVED_FROM_CLASSPATH) > 0
						|| (flags & IRubyElementDelta.F_CONTENT) > 0)
				{
	...
	/**
	 * Returns true if the given type delta (a compilation unit delta or a class file delta) could affect this type
	 * hierarchy.
	 */
	...
	protected static byte[] readUntil(InputStream input, byte separator, int offset) throws IOException,
			RubyModelException
	{
		int length = 0;
	...
	/**
	 * Returns <code>true</code> if an equivalent package fragment is included in the package region. Package fragments
	 * are equivalent if they both have the same name.
	 */
	...
	/**
	 * @see ITypeHierarchy TODO (jerome) should use a PerThreadObject to build the hierarchy instead of synchronizing
	...
	public synchronized void refresh(IProgressMonitor monitor) throws RubyModelException
	{
		try
		{
			...
			if (monitor != null)
			{
				if (this.focusType != null)
				{
					monitor.beginTask(Messages.bind(Messages.hierarchy_creatingOnType, this.focusType
	...
	public void store(OutputStream output, IProgressMonitor monitor) throws RubyModelException
	{
		try
		{
			...
			IType[] types = (IType[]) this.classToSuperclass.keySet().toArray();
			for (IType t : types)
			{
			...
			types = (IType[]) this.typeToSuperModules.keySet().toArray();
			for (IType t : types)
			{
				if (hashtable.get(t) == null)
				{
				...
				IType[] sp = this.typeToSuperModules.get(t);
				if (sp != null)
				{
			...
			// save superclasses
			types = (IType[]) this.classToSuperclass.keySet().toArray();
			for (int i = 0; i < types.length; i++)
			{
				IRubyElement key = types[i];
				IRubyElement value = this.classToSuperclass.get(key);
			...
			{
				IRubyElement key = types[i];
				IRubyElement[] values = this.typeToSuperModules.get(key);
	...
	/**
	 * Returns whether the simple name of a supertype of the given type is the simple name of one of the subtypes in
	 * this hierarchy or the simple name of this type.
	 */
	boolean subtypesIncludeSupertypeOf(IType type)
	{
		...
		int dot = -1;
		String simpleSuper = (dot = superclassName.lastIndexOf('.')) > -1 ? superclassName.substring(dot + 1)
				: superclassName;
	...
	public String toString()
	{
		...
		buffer.append(this.focusType == null ? "<NONE>" : ((RubyElement) this.focusType)
	...
	/**
	 * Append a String to the given buffer representing the hierarchy for the type, beginning with the specified
	 * indentation level. If ascendant, shows the super types, otherwise show the sub types.
	 */
