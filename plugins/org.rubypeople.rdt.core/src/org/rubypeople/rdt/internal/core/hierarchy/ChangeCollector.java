...
public class ChangeCollector
{
	 ...
	 */
	private Map<IRubyElement, SimpleDelta> changes = new HashMap<IRubyElement, SimpleDelta>();
	...
	private void addTypeChange(IType type, int newFlags, SimpleDelta existingDelta) throws RubyModelException
	{
		if (existingDelta != null)
		{
			switch (existingDelta.getKind())
			{
				case IRubyElementDelta.CHANGED:
					...
					boolean hasChange = false;
					if ((existingFlags & IRubyElementDelta.F_SUPER_TYPES) != 0 && hasSuperTypeChange(type))
					{
						existingDelta.superTypes();
						hasChange = true;
					}
		...
		else
		{
			...
			if ((newFlags & IRubyElementDelta.F_SUPER_TYPES) != 0 && this.hierarchy.includesTypeOrSupertype(type))
			{
	 ...
	 */
	private void getAllTypesFromElement(IRubyElement element, ArrayList<IType> allTypes) throws RubyModelException
	{
	 ...
	 */
	private void getAllTypesFromHierarchy(RubyElement element, ArrayList<IType> allTypes)
	{
		switch (element.getElementType())
		{
			case IRubyElement.SCRIPT:
			...
			case IRubyElement.FIELD:
			case IRubyElement.METHOD:
				types = this.hierarchy.files.get(((IMember) element).getRubyScript());
				if (types != null)
				{
	...
	public String toString()
	{
		...
		Iterator<Map.Entry<IRubyElement, SimpleDelta>> iterator = this.changes.entrySet().iterator();
		while (iterator.hasNext())
		{
			Map.Entry<IRubyElement, SimpleDelta> entry = (Map.Entry<IRubyElement, SimpleDelta>) iterator.next();
