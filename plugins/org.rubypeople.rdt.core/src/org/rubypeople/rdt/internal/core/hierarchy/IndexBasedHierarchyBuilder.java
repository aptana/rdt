...
public class IndexBasedHierarchyBuilder extends HierarchyBuilder {
	...
	private void buildFromPotentialSubtypes(String[] allPotentialSubTypes, HashSet<String> localTypes, IProgressMonitor monitor) {
		...
		HashMap<String, IRubyScript> wcPaths = new HashMap<String, IRubyScript>(); // a map from path to working copies
		int wcLength;
	...
	private void buildForProject(RubyProject project, ArrayList<IOpenable> potentialSubtypes, org.rubypeople.rdt.core.IRubyScript[] workingCopies, HashSet<String> localTypes, IProgressMonitor monitor) throws RubyModelException {
...
//						}
					...
					// local or anonymous type
					IRubyScript openable = declaringMember.getRubyScript();
					localTypes = new HashSet<String>();
					localTypes.add(openable.getPath().toString());
					this.hierarchyResolver.resolve(new IOpenable[] {openable}, localTypes, monitor);
					return;
				}
			}
	 ...
	 */
	private static void searchAllPossibleSubTypes(
		IType type,
