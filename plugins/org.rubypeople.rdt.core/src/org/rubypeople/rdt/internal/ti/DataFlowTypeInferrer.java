...
public class DataFlowTypeInferrer implements ITypeInferrer
{
	...
	private List<ITypeGuess> getLocalVarReferenceNodeTypes(LocalVarNode node)
	{
		...
		CopyOnWriteArrayList<Node> localAssignsIntoNode = new CopyOnWriteArrayList<Node>(ScopedNodeLocator.Instance().findNodesInScope(enclosingScopeNode,
				new INodeAcceptor()
				{
		...
		if ((localAssignsIntoNode != null) && (localAssignsIntoNode.size() > 0))
		{
			...
			{
				possibleTypes.addAll(inferNodeType(((AssignableNode) asgnNode).getValueNode()));
			}
	...
	private List<ITypeGuess> getCallNodeTypes(Node node)
	{
		...
		possibleTypes.addAll(TypicalMethodReturnNames.get(methodName));		

		List<String> receiverTypes = new ArrayList<String>();
		...
		{
			String receiverTypeName = helper.getTypeNodeName(findEnclosingTypeNode(node));
		...
		List<Node> defnNodes = new ArrayList<Node>();		
		for (String receiverType : receiverTypes)
		{
			// TODO: Find method defnnode, sum types of its retval-exprs
			List<Node> result = findAllMethodDefinitions(receiverType, methodName);
			defnNodes.addAll(result);
			sysout("Receiver type name: " + receiverType);
			sysout(" " + result.size() + " defnnodes found");
		}
