...
public class SelectionEngine
{
	...
	/**
	 * Combines all the resolved elements from all resolvers. FIXME Need to make it so any resolver can modify the
	 * existing list (add or remove)!
	 * 
	 * @param script
	 * @param start
	 * @param end
	...
	public IRubyElement[] select(IRubyScript script, int start, int end) throws RubyModelException
	{
		...
		for (CodeResolver resolver : resolvers)
		{
			...
			{
				stats = PerformanceStats.getStats(PERFORMANCE_EVENT, resolver);
	...
	private List<CodeResolver> getResolvers()
	{
		if (fResolvers == null)
		{
			...
			List<IConfigurationElement> elements = new ArrayList<IConfigurationElement>(Arrays.asList(registry
					.getConfigurationElementsFor(RubyCore.PLUGIN_ID, EXTENSION_POINT)));
