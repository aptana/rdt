...
import org.rubypeople.rdt.internal.formatter.rewriter.ClassBodyWriter;
import org.rubypeople.rdt.internal.formatter.rewriter.Operators;
...
public class ReWriteVisitor implements NodeVisitor
{
	...
	{
		this(new ReWriterContext(new PrintWriter(out), source, new EditableFormatHelper()));
	}

	public ReWriteVisitor(OutputStream out, String source)
	{
		this(new ReWriterContext(new PrintWriter(out, true), source, new EditableFormatHelper()));
	}
	...
	private void printCommentsBefore(Node iVisited)
	{
		...
		for (CommentNode n : iVisited.getComments())
		{
			...
			{
				visitNode(n);
				printComment(n.getContent());
				printNewlineAndIndentation();
			}
		}
	}

	private void printComment(String content)
	{
		if (content == null)
			return;
		if (content.trim().endsWith("=end") && content.startsWith("begin"))
		{
			print('=');
		}
		print(content);
	...
	private void printCallArguments(Node argsNode, Node iterNode)
	{
		...
		if (argsNode != null && argsNode.childNodes().size() == 1 && firstChild(argsNode) instanceof HashNode
				&& iterNode == null)
		{
	...
	private ArrayList<Node> collectAllArguments(ArgsNode iVisited)
	{
		...
		{
			arguments.add(new ConstNode(iVisited.getRestArgNode().getPosition(), '*' + iVisited.getRestArgNode()
					.getName()));
		}
	...
	{
		print("begin");
		if (getStartLine(iVisited) == getEndLine(iVisited.getBodyNode())) // one-liner
		{
			config.setSkipNextNewline(true);
			print(" ");
	...
	public Object visitCaseNode(CaseNode iVisited)
	{
		...
		visitNode(iVisited.getCases().get(0));
		printNewlineAndIndentation();
		print("end");
		return null;
	}
	...
	{
		if (!(iVisited instanceof Colon2ImplicitNode))
		{
			print("::");
		}
		print(iVisited.getName());
	...
	{
		if (firstChild(iVisited) instanceof StrNode)
		{
			StrNode str = (StrNode) firstChild(iVisited);
			// look for a here-document:
			String realSource = getStringSource(str);
			if (realSource != null && realSource.startsWith("<<"))
			{
				print(realSource.trim());
				return null;
			}
		}
	...
	private boolean hasArguments(Node n)
	{
		if (n instanceof ArgsNode)
		{
			...
			return (args.getPre() != null || args.getOptArgs() != null || args.getBlock() != null || args
					.getRestArgNode() != null);
		}
	...
	public Object visitFCallNode(FCallNode iVisited)
	{
		...
		// If the function matches a local variable name and has no args, we should print parens anyways!
		if ((iVisited.getArgsNode() == null || iVisited.getArgsNode().childNodes().isEmpty())
				&& matchingLocalVar(iVisited.getName()))
		{
			print("()");
		}
	...
	private boolean matchingLocalVar(String name)
	{
		for (String varName : config.getLocalVariables().getNames())
		{
	...
	private void printAsgnNode(AssignableNode n)
	{
		...
		// If the value node's receiver is the same name and value node is a call to '+', handle specially!
		if (config.getFormatHelper().collapseOperatorSelfAssignments())
		{
			if (n.getValueNode() instanceof CallNode)
			{
				CallNode call = (CallNode) n.getValueNode();
				if (call.getName().equals("+") || call.getName().equals("-") || call.getName().equals("*"))
				{
					if (call.getReceiverNode() instanceof INameNode)
					{
						INameNode recvr = (INameNode) call.getReceiverNode();
						if (recvr.getName().equals(name))
						{
							print(config.getFormatHelper().beforeAssignment());
							print(call.getName());
							print("=");
							printCallArguments(call.getArgsNode(), call.getIterNode());
							return;
						}
					}
				}
	...
	/**
	 * Elsif-conditions in the AST are represented by multiple nested if / else combinations. This method takes a node
	 * and checks if the node is an elsif-statement or a normal else node.
	 * 
	 * @param iVisited
	...
	{
		return (n instanceof DAsgnNode || n instanceof GlobalAsgnNode || n instanceof InstAsgnNode
				|| n instanceof LocalAsgnNode || n instanceof ClassVarAsgnNode);
	}
	...
	private boolean isShortIfStatement(IfNode iVisited)
	{
		return (isOnSingleLine(iVisited.getCondition(), iVisited.getElseBody())
				&& !(iVisited.getElseBody() instanceof IfNode) && !sourceSubStringEquals(getStartOffset(iVisited), 2,
				"if"));
	}
	...
	public Object visitIfNode(IfNode iVisited)
	{
		...
		if (isIfModifier(iVisited))
			return printIfModifier(iVisited);
	...
	private Object printIfModifier(IfNode iVisited)
	{
		visitNode(iVisited.getThenBody());
		print(" if ");
		if (isAssignment(iVisited.getCondition()))
	...
	private boolean isIfModifier(IfNode visited)
	{
		// condition is after then
		Node then = visited.getThenBody();
		if (then == null)
			return false;
		Node condition = visited.getCondition();
		if (condition.getPosition().getStartOffset() > then.getPosition().getEndOffset())
			return true;
	...
	public Object visitMultipleAsgnNode(MultipleAsgnNode iVisited)
	{
		...
		{
			factory.createMultipleAssignmentReWriteVisitor().visitAndPrintWithSeparator(
					iVisited.getHeadNode().childNodes().iterator());
		}
	...
	public Object visitMultipleAsgnNode(MultipleAsgn19Node iVisited)
	{
		...
		{
			factory.createMultipleAssignmentReWriteVisitor().visitAndPrintWithSeparator(
					iVisited.getPre().childNodes().iterator());
		}
		if (iVisited.getValueNode() == null || iVisited.getValueNode().isInvisible())
		{
	...
	public Object visitNotNode(NotNode iVisited)
	{
		...
		{
			CallNode call = (CallNode) iVisited.getConditionNode();
			String name = call.getName();
			if (name.equals("=="))
			{
				printCallReceiverNode(call);
				print(" !=");
				if (firstChild(call.getArgsNode()) instanceof NewlineNode)
					print(' ');
	...
	{
		return (n instanceof LocalVarNode || n instanceof AssignableNode || n instanceof InstVarNode
				|| n instanceof ClassVarNode || n instanceof GlobalVarNode || n instanceof ConstDeclNode
	...
	{
		if (!iVisited.getBodyNode().isInvisible()
				&& config.getLastPosition().getStartLine() == getEndLine(iVisited.getBodyNode()))
		{
			print(" rescue ");
		}
		else
	...
	public Object visitRescueNode(RescueNode iVisited)
	{
		...
		{
			if (getStartLine(iVisited) != getEndLine(iVisited.getRescueNode().getBodyNode()))
				printNewlineAndIndentation();
	...
	{
		String realSource = getStringSource(iVisited);
		// look for a here-document:
		if (realSource != null && realSource.startsWith("<<"))
		{
			print(realSource.trim());
		...
		if (realSource == null)
			realSource = iVisited.getValue().toString();
		
		// opening and closing of strings
		String opening = "" + getSeparatorForStr(iVisited);
		String closing = opening;
		
		if (realSource != null && realSource.startsWith("%"))
		{
			// Special % string! Keep track of the opening and closing delimiters and print them before and after string contents
			int index = realSource.indexOf(iVisited.getValue().toString());
			opening = realSource.substring(0, index);
			closing = realSource.substring(index + iVisited.getValue().toString().length()).trim();
		}

		if (realSource.equals(""))
		{
			if (config.getPrintQuotesInString().isTrue())
				print("\"\"");
		...
		else
		{
			...
			// FIXME Don't escape " inside '' string
			// FIXME Don't escape ' inside "" string
			// FIXME Don't replace newlines with \n?
			if (matcher.find())
			{
	...
	private String getStringSource(StrNode n)
	{
		String src = config.getSource();
		if (src == null || src.length() == 0)
			return null;
		int start = Math.max(0, n.getPosition().getStartOffset());
		int end = Math.min(src.length(), n.getPosition().getEndOffset() + 1);
		String realValue = src.substring(start, end);
		return realValue;
	}
	...
	{
		return n.getArgsNode().childNodes().isEmpty() && config.getSource().charAt(getEndOffset(n)) == '(';
	}
	...
	{
		if (isUntilModifier(iVisited))
			return printUntilModifier(iVisited);
	...
	private Object printUntilModifier(UntilNode iVisited)
	{
		visitNode(iVisited.getBodyNode());
		print(" until ");
		visitNode(iVisited.getConditionNode());
	...
	private boolean isUntilModifier(UntilNode visited)
	{
		Node condition = visited.getConditionNode();
		Node body = visited.getBodyNode();
		return (body.getPosition().getEndOffset() < condition.getPosition().getStartOffset());
	}
	...
	public Object visitWhenNode(WhenNode iVisited)
	{
		...
		print("when ");
		enterCall();
		Node expression = iVisited.getExpressionNodes();
		if (expression instanceof ListNode)
		{
			visitAndPrintWithSeparator(expression.childNodes().iterator());
		}
		else
		{
	...
	{
		if (isWhileModifier(iVisited))
		{
			printWhileModifier(iVisited);
			return;
		}
		...
		if (config.getFormatHelper().insertDoAfterWhileExpression())
			print(" do");
	...
	private void printWhileModifier(WhileNode visited)
	{
		visitNode(visited.getBodyNode());
		print(" while ");

		if (isAssignment(visited.getConditionNode()))
			enterCall();

		visitNewlineInParentheses(visited.getConditionNode());

		if (isAssignment(visited.getConditionNode()))
			leaveCall();
	}

	private boolean isWhileModifier(WhileNode visited)
	{
		Node condition = visited.getConditionNode();
		Node body = visited.getBodyNode();
		return (body.getPosition().getEndOffset() < condition.getPosition().getStartOffset());
	...
	{
		return createCodeFromNode(node, document, new EditableFormatHelper());
	}
