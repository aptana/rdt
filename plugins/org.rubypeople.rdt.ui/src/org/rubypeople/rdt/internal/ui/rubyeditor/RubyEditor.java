...
public class RubyEditor extends RubyAbstractEditor implements IRubyReconcilingListener
{
	...
	protected void createActions()
	{
		...
		action = new TextOperationAction(RubyEditorMessages.getBundleForConstructedKeys(), "Comment.", this,
				ITextOperationTarget.PREFIX);
		action.setActionDefinitionId(IRubyEditorActionDefinitionIds.COMMENT);
		setAction("Comment", action);

		action = new TextOperationAction(RubyEditorMessages.getBundleForConstructedKeys(), "Uncomment.", this,
		...
		action = new ToggleCommentAction(RubyEditorMessages.getBundleForConstructedKeys(), "ToggleComment.", this); //$NON-NLS-1$
		...
		action = new FormatAction(RubyEditorMessages.getBundleForConstructedKeys(), "Format.", this);
	...
	public void createPartControl(Composite parent)
	{
		...
		if (isTabConversionEnabled())
			startTabConversion();
		startStringSubstitutionConverter();
		...
		public void verifyKey(VerifyEvent event)
		{
			...
			try
			{
				...
				ITypedRegion partition = TextUtilities.getPartition(document, IRubyPartitions.RUBY_PARTITIONING,
						offset - 1, true);
				if (event.character == '{' && previous != null && previous.endsWith("#") && partition != null
						&& partition.getType() != null)
				{
	...
	{
		if (fStringConverter == null)
		{
			fStringConverter = new StringSubstitutionConverter(this);
			...
			if (provider instanceof IRubyScriptDocumentProvider)
			{
				...
				fStringConverter.setLineTracker(cup.createLineTracker(getEditorInput()));
			}
			AdaptedSourceViewer asv = (AdaptedSourceViewer) getSourceViewer();
			asv.addTextConverter(fStringConverter);
		}
	}
