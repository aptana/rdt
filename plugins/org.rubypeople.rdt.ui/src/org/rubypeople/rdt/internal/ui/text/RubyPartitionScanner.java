...
public class RubyPartitionScanner implements IPartitionTokenScanner
{
		...
		try
		{
			...
			else
			{
				...
				else if (lexerToken == Tokens.tSTRING_BEG)
				{
					...
					if (opening.trim().startsWith("<<") && endOfMarker != -1)
					{
	...
	private void setLexerPastDynamicSectionOfString() throws IOException
	{
		...
		if (opening.endsWith("\n"))
		{
			...
			// try searching backwards from fOffset in fContents for <<-opening or <<opening and take whichever we find
			// first. If we fail to find, assume <<
			String heredocStart = "<<";
			int lastIndent = fContents.lastIndexOf("<<-" + opening, fOffset);
			if (lastIndent != -1)
			{
				if (lastIndent > fContents.lastIndexOf("<<" + opening, fOffset))
					heredocStart = "<<-";
			}
	...
	private IToken getToken(int i)
	{
		...
		switch (i)
		{
			...
			case Tokens.tREGEXP_END:
				...
				return new Token(RUBY_REGULAR_EXPRESSION);
			case Tokens.tSYMBEG:
				// Sometimes we need to add 1, sometimes two. Depends on if there's a space preceding the ':'
	...
	{
		setPartialRange(document, offset, length, RUBY_DEFAULT, 0);
	}
			...
			for (int i = 0; i < input.length(); i++)
			{
				...
				switch (c)
				{
					...
					case '\'':
						...
						else if (!topEquals("\"") && !topEquals("/")) // not inside a double quoted string or a regex
						{
							push("'");
						}
						break;
