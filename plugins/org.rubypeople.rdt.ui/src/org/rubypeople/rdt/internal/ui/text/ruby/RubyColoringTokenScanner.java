...
/**
 * Wraps the RubyTokenScanner, and converts it's integer tokens into tokens containing coloring ifnromation for the ruby
 * editor.
 * 
...
public class RubyColoringTokenScanner extends AbstractRubyTokenScanner
{
	...
	private static String[] fgTokenProperties = { IRubyColorConstants.RUBY_KEYWORD, IRubyColorConstants.RUBY_DEFAULT,
			...
			IRubyColorConstants.RUBY_CLASS_VARIABLE, IRubyColorConstants.RUBY_INSTANCE_VARIABLE,
			IRubyColorConstants.RUBY_GLOBAL, IRubyColorConstants.RUBY_ERROR
	...
	public IToken nextToken()
	{
		...
		switch (data.intValue())
		{
			...
			case Tokens.tCVAR:
				return getToken(IRubyColorConstants.RUBY_CLASS_VARIABLE);
	...
	{
		if (i >= RubyTokenScanner.MIN_KEYWORD && i <= RubyTokenScanner.MAX_KEYWORD)
			return true;
		return false;
	}
