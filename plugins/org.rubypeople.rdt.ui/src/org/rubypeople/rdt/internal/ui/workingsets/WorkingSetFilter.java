...
public class WorkingSetFilter extends ViewerFilter {
	...
	public boolean select(Viewer viewer, Object parentElement, Object element) {
		if (fWorkingSet == null
				|| (fWorkingSet.isAggregateWorkingSet() && fWorkingSet
		...
		if (element instanceof IAdaptable) {
			...
			IRubyElement je = (IRubyElement) adaptable
	...
	public boolean isEnclosing(IRubyElement element) {
		...
		int length = cachedWorkingSet.length;
		for (int i = 0; i < length; i++) {
			IRubyElement scopeElement = (IRubyElement) cachedWorkingSet[i]
					.getAdapter(IRubyElement.class);
			if (scopeElement != null) {
				...
				while (searchedElement != null) {
					if (searchedElement.equals(scopeElement))
						...
						if (scopeElement.getElementType() == IRubyElement.RUBY_PROJECT
								&& searchedElement.getElementType() == IRubyElement.SOURCE_FOLDER_ROOT) {
							ISourceFolderRoot pkgRoot = (ISourceFolderRoot) searchedElement;
							if (pkgRoot.isExternal() && pkgRoot.isArchive()) {
								if (((IRubyProject) scopeElement)
										.isOnLoadpath(searchedElement))
									return true;
							}
						}
						if (searchedElement.getElementType() == IRubyElement.SOURCE_FOLDER
								&& scopeElement.getElementType() == IRubyElement.SOURCE_FOLDER) {
							IPath searchPath = searchedElement.getPath();
							IPath scopePath = scopeElement.getPath();
							ISourceFolderRoot root = (ISourceFolderRoot) searchedElement
									.getParent();
							IPath rootPath = root.getPath();
							IPath parentFolderPath = searchPath
									.removeFirstSegments(
											rootPath.segmentCount())
									.removeLastSegments(1);
							searchedElement = root
									.getSourceFolder(parentFolderPath
											.segments());
						} else {
							searchedElement = searchedElement.getParent();
						}
						if (searchedElement != null
								&& searchedElement.getElementType() == IRubyElement.SCRIPT) {
