...
public class CodeReloader implements IPartListener, IPropertyListener, IWindowListener
{
	...
	public void propertyChanged(Object source, int propId)
	{
		...
		for (int i = 0; i < launches.length; i++)
		{
			...
			{
				RdtDebugUiPlugin.log(new Status(IStatus.ERROR, RdtDebugUiPlugin.PLUGIN_ID, 0,
						"Error while trying to reload file.", e));
			}
		}
	}
	...
	public CodeReloader()
	{
		...
		UIJob job = new UIJob("")
		{
			...
			{
				RdtDebugUiPlugin.getDefault().getWorkbench().addWindowListener(self);
				IWorkbenchWindow[] windows = RdtDebugUiPlugin.getDefault().getWorkbench().getWorkbenchWindows();
				for (int i = 0; i < windows.length; i++)
				{
					IWorkbenchWindow window = windows[i];
					window.getPartService().addPartListener(self);
				}
