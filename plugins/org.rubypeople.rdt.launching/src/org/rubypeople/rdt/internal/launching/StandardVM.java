...
public class StandardVM extends AbstractVMInstall
{
	...
	public IPath[] getLibraryLocations()
	{
		...
		if (paths != null)
		{
			// TODO Make sure the core stubs are generated!
			generateCoreStubs(getVMInstallType().findExecutable(getInstallLocation()));
			return paths;
		}
	...
	private IPath[] getDefaultLibraryLocations()
	{
		...
		IPath coreStubsPath = generateCoreStubs(getVMInstallType().findExecutable(getInstallLocation()));
	...
	/**
	 * Launch a ruby script to generate core class stubs for use in RDT internally (since Ruby core stuff is not in any
	 * scripts, they're built into the VM in C code).
	 * 
	...
	private IPath generateCoreStubs(final File rubyExecutable)
	{
		...
		final File coreStubber = LaunchingPlugin
				.getFileInPlugin(new Path("ruby").append("standard_vm").append("core_stubber.rb")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		...
		if (stubFolder.toFile().exists() && stubFolder.append(FINISHED_MARKER).toFile().exists()) // finished before
		{
		...
		if (coreStubJob != null)
		{
			IStatus result = coreStubJob.getResult();
			if (result == null)
			{ // currently running. Let's just return...
				return stubFolder;
			}
			// job has finished. let's kill it and re-schedule because something went wrong.
			coreStubJob.cancel();
		}
		coreStubJob = new Job("Generating core library stubs...")
		{
			...
			protected IStatus run(IProgressMonitor monitor)
			{
				...
				// FIXME JRuby core stub generation is broken in windows!!!
				rubyExecutablePath = rubyExecutablePath.replace("rubyw", "ruby"); // Use ruby.exe, not rubyw!
				String[] cmdLine = new String[] { rubyExecutablePath, coreStubber.getAbsolutePath(),
						stubFolder.toOSString() };
				...
				try
				{
					...
					IProcess process = DebugPlugin.newProcess(new Launch(null, ILaunchManager.RUN_MODE, null), p,
							"Core Classes Stub Generation"); //$NON-NLS-1$
					...
					if (exitValue == 0)
					{
						// Add file which just marks we were able to finish
						stubFolder.append(FINISHED_MARKER).toFile().createNewFile();
					}
				}
