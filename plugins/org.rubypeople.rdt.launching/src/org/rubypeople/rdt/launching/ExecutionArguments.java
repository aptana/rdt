 ...
 * separated into two parts: arguments to the VM itself, and arguments to the
 * Java main program. This class provides convenience methods for parsing a
 * string of arguments into separate components.
 * <p>
...
public class ExecutionArguments {
	 ...
	 * 
	 * @param vmArgs
	 *            command line argument string passed to the VM
	 * @param programArgs
	...
	public String[] getVMArgumentsArray() {
		String[] raw = DebugPlugin.parseArguments(fVMArgs);
		List<String> modified = new ArrayList<String>();
		for (int i = 0; i < raw.length; i++) {
			String arg = raw[i];
			if ((arg.equals("-e") || arg.equals("-X") || arg.equals("-F"))
					&& (raw.length > (i + 1))) {
				modified.add(arg + " " + raw[i + 1]);
				i++;
			} else {
				modified.add(arg);
			}
