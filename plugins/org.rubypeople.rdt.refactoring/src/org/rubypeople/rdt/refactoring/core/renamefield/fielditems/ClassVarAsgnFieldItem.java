...
public class ClassVarAsgnFieldItem extends FieldItem {
	
	private ClassVarAsgnNode classVarAsgnNode;
	
	public ClassVarAsgnFieldItem(ClassVarAsgnNode classVarAsgnNode) {

		this.classVarAsgnNode = classVarAsgnNode;
	}

	@Override
	public String getFieldName() {
		return fieldName(classVarAsgnNode.getName());
	}

	@Override
	public Node getFieldNode() {
		return classVarAsgnNode;
	}
	...
	private void replaceNameInSubnodes(Node baseNode, String newName) {
		if(baseNode instanceof ClassVarNode){
			((ClassVarNode)baseNode).setName(newName);
		}
		for(Object currentChild : baseNode.childNodes()){
			replaceNameInSubnodes((Node)currentChild, newName);
		}
	}
