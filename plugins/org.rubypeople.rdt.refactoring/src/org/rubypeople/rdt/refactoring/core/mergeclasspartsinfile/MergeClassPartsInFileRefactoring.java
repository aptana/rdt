...
import org.rubypeople.rdt.refactoring.ui.pages.MergeClassPartsInFilePage;
...
public class MergeClassPartsInFileRefactoring extends RubyRefactoring {
	...
	public MergeClassPartsInFileRefactoring() throws CoreException {
		...
		MergeClassPartInFileConfig config = new MergeClassPartInFileConfig(getDocumentProvider());
		...
		if(checker.shouldPerform()) {
			...
			setEditProvider(selector);

			MergeClassPartsInFilePage selectionPage = new MergeClassPartsInFilePage(config);

			pages.add(selectionPage);
		}
	}
}
