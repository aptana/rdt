...
public class ConstructorCall {
	...
	public String getClassName() {
		return NameHelper.getFullyQualifiedName(node.getReceiverNode());
	}
	...
	public void setName(String newName) {
		if(node.getReceiverNode() instanceof ConstNode) {
			((ConstNode) node.getReceiverNode()).setName(newName);
		} else {
			((Colon2Node) node.getReceiverNode()).setName(newName);
		}
	}
	public int getReceiverOffset() {
		return node.getReceiverNode().getPosition().getStartOffset();
	}
	public int getReceiverLength() {
		return node.getReceiverNode().getPosition().getEndOffset() - getReceiverOffset(); 
	}
}
