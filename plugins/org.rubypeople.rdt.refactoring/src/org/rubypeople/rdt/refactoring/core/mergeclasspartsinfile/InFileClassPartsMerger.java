...
public class InFileClassPartsMerger extends MultiEditProvider {


	private MergeClassPartInFileConfig config;

	public InFileClassPartsMerger(MergeClassPartInFileConfig config) {
		this.config = config;
	}
	...
	protected Collection<EditProvider> getEditProviders() {
		...
		EditProviderGroups providerGroups = new EditProviderGroups();

		for (PartialClassNodeWrapper currentClassPart : config.getCheckedClassParts()) {
			if (currentClassPart == config.getSelectedClassPart()) {
				continue;
			}
			DeleteEditProvider delEditProvider = new DeleteEditProvider(currentClassPart.getWrappedNode());
		...
		return providerGroups.getAllEditProviders();
	}
}
