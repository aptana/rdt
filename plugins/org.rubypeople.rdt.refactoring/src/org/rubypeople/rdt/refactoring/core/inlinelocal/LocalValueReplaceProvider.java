...
public class LocalValueReplaceProvider extends ReplaceEditProvider
{
	...
	{
		Node targetEnclosingNode = NodeProvider.findParentNode(config.getDocumentProvider().getActiveFileRootNode(),
				targetNode.getWrappedNode());
		boolean isTargetEnclosingNodeCallNode = NodeUtil.nodeAssignableFrom(targetEnclosingNode, MethodCallNodeWrapper
				.METHOD_CALL_NODE_CLASSES());
		...
		boolean isInlinedNodeCallNode = NodeUtil.nodeAssignableFrom(inlinedNode.getValueNode(), MethodCallNodeWrapper
				.METHOD_CALL_NODE_CLASSES());
