...
import org.rubypeople.rdt.refactoring.ui.pages.MergeWithExternalClassPartsPage;
...
public class MergeWithExternalClassPartsRefactoring extends RubyRefactoring {
	...
	public MergeWithExternalClassPartsRefactoring() {
		super(NAME);
		
		MergeWithExternalClassPartConfig config = new MergeWithExternalClassPartConfig(getDocumentProvider());
		...
		if(checker.shouldPerform()) {
			merger = new ExternalClassPartsMerger(config);
			setEditProvider(merger);
			MergeWithExternalClassPartsPage page = new MergeWithExternalClassPartsPage(merger);
			pages.add(page);
		}
	}
}
