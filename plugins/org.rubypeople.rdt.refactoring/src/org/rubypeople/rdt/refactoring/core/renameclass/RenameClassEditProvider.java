...
public class RenameClassEditProvider implements IMultiFileEditProvider {
	...
	private ChildClassesRenameEditProvider createChildrenEditProvider() {
		Collection<ClassNode> childClasses = new ClassFinder(document, config.getOldName(), config.getModulePrefix()).findChildren();
	...
	private ScopingNodeRenameEditProvider createPartialsEditProvider() {
		Collection<ClassNode> classNodes = new ClassFinder(document, config.getOldName(),  config.getModulePrefix()).findParts();
	...
	private ConstructorRenameEditProvider createConstructorEditProvider() {
		Collection<ConstructorCall> allCalls = new ClassInstanciationFinder().findAll(document, config.getOldName(), config.getModulePrefix());
	...
	public Collection<FileMultiEditProvider> getFileEditProviders() {
		...
		// Order is important because the partials edit provider actually modifies the shared node's name!!
