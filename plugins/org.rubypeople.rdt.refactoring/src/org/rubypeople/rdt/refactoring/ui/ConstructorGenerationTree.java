...
public class ConstructorGenerationTree extends CheckboxTreeViewer {
	...
	protected void setChildrenUnchecked(IChildrenProvider item) {
		for (Object attr : item.getChildren()) {
			setChecked(attr, false);
			setItemChecked((CheckableItem) attr, false);
		}
	}

	protected void setParentChecked(IParentProvider item) {
		setChecked(item.getParent(), true);
		setItemChecked((CheckableItem) item.getParent(), true);
	}
}
