...
public class TC_ParameterReplacer extends FinderTestsBase {

	public TC_ParameterReplacer(String fileName) {
		super(fileName);
	}
	...
	public void runTest() throws FileNotFoundException, IOException {
		...
		IDocumentProvider document = new ParameterReplacer().replace(testData, node, definitionNode);
		assertNotNull(document);
		
		assertEquals(testData.getExpectedResult().trim(), document.getActiveFileContent());
	}
}
