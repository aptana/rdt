...
package org.rubypeople.rdt.refactoring.tests.core.renamemethod;
...
public class MethodRenamerTester extends RefactoringTestCase {
	...
	public void runTest() throws FileNotFoundException, IOException, BadLocationException {
		...
		IRefactoringConditionChecker checker = new RenameMethodConditionChecker(config);
		...
		if (testData.getBoolProperty("renameCalls")) {
			Collection<INodeWrapper> renamedCalls = new ArrayList<INodeWrapper>();
			if(testData.getBoolProperty("renameAllCalls")){
				renamedCalls.addAll(config.getPossibleCalls());
			} else{				
				renamedCalls.addAll(renamer.getCallCandidatesInClass());
				renamedCalls.addAll(renamer.getSubsequentCalls());
			}
			config.setSelectedCalls(renamedCalls);
			System.out.println();
		}
